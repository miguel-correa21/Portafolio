...
DEVUELVE:nada.
esta función se encarga de crear un arbol por cada simbolo diferente
 y los junta hasta que acomplete el arbol binario*/
void CrearArbol(elemento tablaFR[])
{
	...
	{
		Initialize(&hojas[i]);
		NewRightSon(&hojas[i],aux,tablaFR[subindices[i]]);/*como el arbol
 * esta vacio se convierte en raiz*/
		aux = Root(&hojas[i]);
		InOrden(&hojas[i],aux);
		i++;
	}
...
DEVUELVE:nada
esta función se encarga de ordenar de menor a mayor el arreglo de arboles,
 mediante el mértodo burbuja*/
...
auxiliar y asigna los 0's y 1's en el arbol,
 el tope del arreglo, y la poscion en la que se encuentra.
DEVUELVE: nada
...
DEVUELVE:nada
esta función se encarga de descomprimir y guardar en un nuevo archivo la

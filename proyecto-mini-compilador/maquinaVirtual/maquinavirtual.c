...
/******* const *******/
#define   IADDR_SIZE  1024 /* incrementar para programas largos */
#define   DADDR_SIZE  1024 /* incrementar para programas largos */
#define   NO_REGS 8
#define   PC_REG  7
...
typedef enum {
   ...
   opSALIDA,     /* RR     write from reg(r), s and t are ignored */
   opSUMA,    /* RR     reg(r) = reg(s)+reg(t) */
   opRESTA,    /* RR     reg(r) = reg(s)-reg(t) */
...
char * opCodeTab[]
        = {"DETENER","IN","SALIDA","SUMA","SUB","MUL","DIV","????",
            /* RR opcodes */
           "LD","ST","????", /* RM opcodes */
...
char * stepResultTab[]
        = {"OK","DETENER","Falla de memoria de instrucciones",
           "Falla de memoria de datos","Division entre 0"
          };
...
{ printf("Linea %d",lineNo);
  if (instNo >= 0) printf(" (Instrucción %d)",instNo);
  printf("   %s\n",msg);
  return FALSE;
} /* error */
...
{ OPCODE op;
  ...
  { fgets( in_Line, LINESIZE-2, pgm  ) ;
    ...
    { if (! getNum())
        return error("Localización erronea", lineNo,-1);
      loc = num;
      if (loc > IADDR_SIZE)
        return error("Locazación muy larga",lineNo,loc);
      if (! skipCh(':'))
        return error("Falta punto y coma", lineNo,loc);
      if (! getWord ())
        return error("Falta opcode", lineNo,loc);
      op = opHALT ;
      while ((op < opRALim)
      ...
      { case opclRR :
        ...
        if ( (! getNum ()) || (num < 0) || (num >= NO_REGS) )
            return error("Falla del primer registro", lineNo,loc);
        arg1 = num;
        if ( ! skipCh(','))
            return error("Falta coma", lineNo, loc);
        if ( (! getNum ()) || (num < 0) || (num >= NO_REGS) )
            return error("Falla del segundo registro", lineNo, loc);
        arg2 = num;
        if ( ! skipCh(',')) 
            return error("Falta coma", lineNo,loc);
        if ( (! getNum ()) || (num < 0) || (num >= NO_REGS) )
            return error("Falla en tercer registro", lineNo,loc);
        arg3 = num;
        break;
        ...
        if ( ! skipCh(','))
            return error("Falta coma", lineNo,loc);
        if (! getNum ())
            return error("Falta  desplazamiento", lineNo,loc);
        arg2 = num;
        if ( ! skipCh('(') && ! skipCh(',') )
            return error("Falta  LParen", lineNo,loc);
        if ( (! getNum ()) || (num < 0) || (num >= NO_REGS))
            return error("Fallo en segundo registro", lineNo,loc);
        arg3 = num;
        break;
        }
      iMem[loc].iop = op;
...
{ INSTRUCTION currentinstruction  ;
  ...
  { /* RR instructions */
    ...
    case opSALIDA :  
      printf ("SALIDA: %d\n", reg[r] ) ;
      break;
    case opSUMA :  reg[r] = reg[s] + reg[t] ;  break;
    case opRESTA :  reg[r] = reg[s] - reg[t] ;  break;
...
{ char cmd;
  ...
  { case 't' :
    ...
    /***********************************/
		  printf("Comandos:\n");
            printf("    s(tep <n>         "\
                    "Executar instrucciones n (por defecto 1) escompis.\n");
            printf("    g(o               "\
                    "Executar instrucciones TM hasta DETENER\n");
            printf("    r(egs             "\
                    "Impresion del contenido del registro.\n");
            printf("    i(Mem <b <n>>     "\
                    "Impresion n iMem direcciones empezando en b.\n");
            printf("    d(Mem <b <n>>     "\
                    "Impresion n dMem direcciones empezando en b.\n");
            printf("    t(race            "\
                    "Rastreo de instrucciones.\n");
            printf("    p(rint            "\
                    "Impresion del total de instrucciones executadas."\
                    " ('go' only)\n");
            printf("    c(lear            "\
                    "Reiniciar el simulador para una nueva ejecución.\n");
            printf("    h(elp            "\
                    "Hacer que se imprima esta lista de comandos.\n");
            printf("    q(uit            "\
    ...
    /***********************************/
	  ...
	  printf("Imprimiendo instrucciones a partir de ahora.");
      if ( icountflag ) printf("Prendido.\n"); else printf("apagado.\n");
      break;
    ...
    /***********************************/
      ...
      if ( ! atEOL ())
        printf ("Localizacion de instrucciones?\n");
      else
  ...
  { if ( cmd == 'g' )
    { stepcnt = 0;
      ...
      if ( icountflag )
        printf("Numero de instruccion ejecutada = %d\n",stepcnt);
    }
    else
...
{ if (argc != 2)
  ...
  /* reset( input ); */
  /* read-eval-print */
  printf("Maquina virtual (ingresa h para ayuda)...\n");
  do
     done = ! doCommand ();
  while (! done );
